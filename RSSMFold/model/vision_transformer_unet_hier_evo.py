...
class UNetVTEncoder(nn.Module):
    ...
    def __init__(self, all_device, layer_split_idx, nb_stride2_downsampling, emb_dim, dropout, patch_ds_stride=20,
                 **kwargs):
    ...
    def forward(self, contact_map, evo_features, padding_mask, enable_checkpoint, conv_backbone=True):
        ...
        # downsampled evo_features
        evo_features = list(map(lambda features: rearrange(
            F.pad(features, (0, contact_map_paddings, 0, contact_map_paddings)),
            'b d (nw_h w_h) (nw_w w_w) -> b d nw_h nw_w (w_h w_w)',
            w_h=self.patch_ds_stride, w_w=self.patch_ds_stride).mean(dim=-1), evo_features))
        ...
        for i in range(self.nb_stride2_downsampling * 2 + 1):
            ...
            patch_map = t_layer_2(t_layer_1(patch_map, evo_features, padding_mask), evo_features, padding_mask)
            ...
            if i < self.nb_stride2_downsampling:  # down-sampling path
                ...
                all_ds_evo_features.append(evo_features)
                ...
                evo_features = list(map(lambda features: rearrange(
                    F.pad(features, (0, padding, 0, padding)),
                    'b d (nw_h w_h) (nw_w w_w) -> b d nw_h nw_w (w_h w_w)',
                    w_h=2, w_w=2).mean(dim=-1), evo_features))
                ...
                evo_features = list(map(lambda features: features.to(patch_map.device), all_ds_evo_features.pop()))
            ...
            if len(self.layer_split_idx) > layer_split_idx and i == self.layer_split_idx[layer_split_idx]:
                ...
                padding_mask = padding_mask.to(device)
                evo_features = list(map(lambda features: features.to(device), evo_features))
...
class VTBlock(nn.Module):
    ...
    def forward(self, src, evo_features, padding_mask=None):
        ...
        if self.use_lw:
            ...
            # the same rearrangement for evo features
            evo_features = list(map(lambda features: rearrange(
                F.pad(features, (patches_l_pad, patches_r_pad, patches_l_pad, patches_r_pad)),
                'b d (nw_h w_h) (nw_w w_w) -> (b nw_h nw_w) d w_h w_w',
                w_h=self.lw_size, w_w=self.lw_size), evo_features))
        ...
        # adding evolutionary information to the attention map
        for features in evo_features:
            features = features.contiguous().view(eff_bsz, self.nhead, -1, eff_attn_field)
            scores = torch.matmul(features.transpose(2, 3), features)
...
class UNetVTModel(nn.Module):
    ...
    def __init__(self, nb_stride2_downsampling, emb_dim, nhead, device, **kwargs):
        ...
        self.map_concat_mode = kwargs.get('map_concat_mode', 'sum')
        assert self.map_concat_mode in ['sum', 'concat']
        ...
        self.nb_post_convs = kwargs.get('nb_post_convs', 10)
        self.enable_5x5_filter = kwargs.get('enable_5x5_filter', False)
        self.include_lp_features = kwargs.get('include_lp_features', False)
        self.include_coupling_features = kwargs.get('include_coupling_features', True)
        self.compute_norm = kwargs.get('compute_norm', False)
        self.transformer_encoder = UNetVTEncoder(
            self.all_device, self.layer_split_idx, self.nb_stride2_downsampling, emb_dim,
            self.t_dropout, patch_ds_stride=self.patch_ds_stride, nhead=4,
        ...
        if self.map_concat_mode == 'sum':
            # embedding layer
            self.x_embedding = nn.Embedding(nb_type_node + 1, emb_dim, padding_idx=nb_type_node).to(self.all_device[0])
            self.evo_mlps = nn.ModuleList([
                nn.Linear(16, self.emb_dim)
            ]).to(self.all_device[0])  # for the covariance features
            if self.include_coupling_features:
                self.evo_mlps.append(nn.Linear(16, self.emb_dim).to(self.all_device[0]))  # gremlin features
                self.evo_mlps.append(nn.Linear(16, self.emb_dim).to(self.all_device[0]))  # mf features
            if self.include_lp_features:
                self.evo_mlps.append(nn.Linear(1, self.emb_dim).to(self.all_device[0]))
        else:
            # 8 for nucleotide, 16 for cov, 16/1 for grem, 16/1 for mf and 1 for lp
            if self.include_coupling_features:
                if self.compute_norm:
                    concat_feature_dim = 27 if self.include_lp_features else 26
                else:
                    concat_feature_dim = 57 if self.include_lp_features else 56
            else:
                concat_feature_dim = 25 if self.include_lp_features else 24

            self.initial_feature_merge_conv = nn.Conv2d(
                concat_feature_dim, emb_dim, 3, stride=1, padding=1).to(self.all_device[0])

        self.ablate_seq = kwargs.get('ablate_seq', False)
        self.ablate_model = kwargs.get('ablate_model', False)
        self.init_weights()
    ...
    def forward(self, x, batch_len, evo_features, enable_checkpoint=True, conv_backbone=False):
        ...
        if self.map_concat_mode == 'sum':
            ...
            pe = get_original_pe(batch_len_np, max_len, self.emb_dim, return_flattened=False).to(self.all_device[0])
            if self.ablate_seq:
                x = pe.unsqueeze(0).repeat(batch_size, 1, 1)
            else:
                x = self.x_embedding(x) + pe  # batch_size, length, dimension

            all_evo_features = []
            for i, feature in enumerate(evo_features):
                feature = self.evo_mlps[i](feature)
                # from flattened triu to matrix
                cumsum_triu_size = 0
                mat_features = []
                for seq_len in batch_len_np:
                    triu_size = (seq_len + 1) * seq_len // 2
                    mat = torch.zeros((max_len, max_len, self.emb_dim), dtype=torch.float32).to(self.all_device[0])
                    mat[np.triu_indices(seq_len)] = feature[cumsum_triu_size: cumsum_triu_size + triu_size]
                    mat[np.triu_indices(seq_len)[::-1]] = feature[cumsum_triu_size: cumsum_triu_size + triu_size]
                    cumsum_triu_size += triu_size
                    mat_features.append(mat)
                mat_features = torch.stack(mat_features, dim=0).permute(0, 3, 1, 2)  # b, d, l, l
                all_evo_features.append(mat_features)

            contact_map = obtain_contact_map('sum', x).permute(0, -1, 1, 2)
            # add evolutionary features to the contact map directly
            contact_map = sum([contact_map] + all_evo_features)

        else:
            # one-hot encoding
            x = torch.eye(7).to(self.all_device[0])[x][:, :, :4]

            cumsum_triu_size = 0
            if self.include_coupling_features:
                if self.compute_norm:
                    evo_feature_dim = 19 if self.include_lp_features else 18
                else:
                    evo_feature_dim = 49 if self.include_lp_features else 48
            else:
                evo_feature_dim = 17 if self.include_lp_features else 16

            mat_features = []
            for seq_len in batch_len_np:
                triu_size = (seq_len + 1) * seq_len // 2
                mat = torch.zeros((max_len, max_len, evo_feature_dim), dtype=torch.float32).to(self.all_device[0])
                mat[np.triu_indices(seq_len)] = evo_features[cumsum_triu_size: cumsum_triu_size + triu_size]
                mat[np.triu_indices(seq_len)[::-1]] = evo_features[cumsum_triu_size: cumsum_triu_size + triu_size]
                cumsum_triu_size += triu_size
                mat_features.append(mat)
            mat_features = torch.stack(mat_features, dim=0).permute(0, 3, 1, 2)  # b, d, l, l

            contact_map = obtain_contact_map('concat', x).permute(0, -1, 1, 2)
            joint_contact_map = torch.cat([contact_map, mat_features], dim=1)
            contact_map = self.initial_feature_merge_conv(joint_contact_map)
        ...
        if self.ablate_model:
            raise RuntimeError('ablate model option is unavailable')
            # contact_map = obtain_contact_map(self.map_concat_mode, x).permute(0, -1, 1, 2)
            # contact_map = contact_map + all_evo_features  # todo
            # contact_map = contact_map.to(self.all_device[-1])
            # all_patch_maps = [(contact_map, batch_len)]
        else:
            all_patch_maps = self.transformer_encoder(
                contact_map, all_evo_features, batch_padding_mask.unsqueeze_(1), enable_checkpoint, conv_backbone)
